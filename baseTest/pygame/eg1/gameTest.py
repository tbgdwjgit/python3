...
def runGame():  # 实际的游戏代码都在runGame中
    ...
    while True:  # 游戏主循环，它负责砖块在落向底部的过程中，游戏主要部分的代码
        ...
        if (
                movingLeft or movingRight) and time.time() - lastMoveSidewaysTime > MOVESIDEWAYSFREQ:  # MOVESIDEWAYSFREQ = 0.15 按向左或向右超过0.15秒
        ...
        if time.time() - lastFallTime > fallFreq:  # fallFreq向下移动的速率
            if not isValidPosition(board, fallingPiece, adjY=1):  # 当砖块下一个位置无效时，即表示砖块当前已经着陆了。
                ...
                level, fallFreq = calculateLevelAndFallFreq(
                    score)  # 由于分数已经修改了，我们调用calculateLevelAndFallFreq()函数来更新当前的关卡以及砖块下落得频率。
...
def drawBoard(board):  # 绘制游戏板边界
    ...
    pygame.draw.rect(DISPLAYSURF, BGCOLOR,
                     (XMARGIN, TOPMARGIN, BOXSIZE * BOARDWIDTH, BOXSIZE * BOARDHEIGHT))  # 填充游戏板的背景颜色
...
def drawBox(boxx, boxy, color, pixelx=None, pixely=None):  # 绘制一个有效方块
    ...
    pygame.draw.rect(DISPLAYSURF, COLORS[color],
                     (pixelx + 1, pixely + 1, BOXSIZE - 1, BOXSIZE - 1))  # 留出1像素的空白，这样才能在砖块中看到组成砖块
